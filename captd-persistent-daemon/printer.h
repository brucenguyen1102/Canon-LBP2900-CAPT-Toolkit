...
struct printer_state_s {
	...
	unsigned iband;
	unsigned isend;
	unsigned cont_ticks_since_send; /* 100ms ticks since last SetJobInfo2(flag=CONT) heartbeat */
	bool sent_job_cont_once;   /* first CONT heartbeat has fired (gates page_completed>=1 start) */
